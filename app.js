...
function render() {
  ...
  app.innerHTML = `
    <main style="min-height:100vh;padding:32px 16px 48px;">
      <div style="max-width:1040px;margin:0 auto;display:grid;gap:24px;">
        <section style="background:rgba(255,252,246,.92);border:1px solid rgba(126,103,71,.14);border-radius:28px;box-shadow:0 24px 60px rgba(89,62,25,.12);overflow:hidden;">
          <div style="padding:32px 24px 20px;background:linear-gradient(135deg, rgba(138,74,56,.12), rgba(231,181,102,.08));border-bottom:1px solid rgba(126,103,71,.1);">
            <div style="font-size:12px;letter-spacing:.16em;text-transform:uppercase;color:#8a6542;margin-bottom:10px;">Family Europe Journey</div>
            <h1 style="margin:0;font-size:34px;line-height:1.08;color:#3b2d22;">Prague, Vienna, Salzburg</h1>
            <p style="margin:12px 0 0;max-width:720px;font-size:15px;line-height:1.7;color:#645548;">An interactive day-by-day itinerary in the same detailed style as your Claude version, with timing, transit, food stops, and expandable insider tips all in one place.</p>
          </div>
          <div style="padding:24px;">
            <div style="overflow-x:auto;display:flex;gap:6px;padding-bottom:10px;margin-bottom:18px;scrollbar-width:none;">
              ${DAYS.map((d, i) => `
                <button data-day="${i}" style="flex-shrink:0;padding:7px 13px;border-radius:99px;border:none;cursor:pointer;font-size:12px;background:${dayIdx === i ? d.accentColor : "#f1efe8"};color:${dayIdx === i ? "#fff" : "#5f5e5a"};font-weight:${dayIdx === i ? 700 : 400};">
                  Day ${d.id}
                </button>
              `).join("")}
            </div>

            <div style="margin-bottom:16px;padding-bottom:14px;border-bottom:2px solid ${day.accentColor}22;">
              <div style="font-size:10px;letter-spacing:.14em;text-transform:uppercase;color:#888780;margin-bottom:3px;">${escapeHtml(day.country)}</div>
              <div style="display:flex;justify-content:space-between;align-items:flex-start;gap:12px;flex-wrap:wrap;">
                <div style="flex:1 1 320px;">
                  <h2 style="font-size:20px;font-weight:700;margin:0 0 2px;color:${day.accentColor};line-height:1.2;">${escapeHtml(day.title)}</h2>
                  <p style="font-size:12px;color:#888780;margin:0;font-style:italic;">${escapeHtml(day.sub)}</p>
                </div>
                <div style="text-align:right;flex-shrink:0;max-width:220px;">
                  <div style="font-size:10px;color:#aaa;margin-bottom:2px;">Sleeping at</div>
                  <div style="font-size:11px;color:#444;line-height:1.4;">${escapeHtml(day.hotelNote)}</div>
                </div>
              </div>
            </div>

            <div style="position:relative;padding-left:64px;">
              <div style="position:absolute;left:48px;top:8px;bottom:8px;width:2px;background:${day.accentColor}25;"></div>
              ${day.events.map((ev, i) => {
                const s = TYPE_STYLES[ev.type] || TYPE_STYLES.walk;
                const isOpen = openIdx === i;
                return `
                  <div style="position:relative;margin-bottom:6px;">
                    <div style="position:absolute;left:-64px;top:14px;font-size:11px;color:#888780;font-family:monospace;width:44px;text-align:right;line-height:1.2;">${escapeHtml(ev.time)}</div>
                    <div style="position:absolute;left:-18px;top:16px;width:10px;height:10px;border-radius:50%;background:${day.accentColor};border:2px solid white;z-index:1;box-shadow:0 0 0 2px ${day.accentColor}30;"></div>
                    <div data-card="${i}" style="background:${s.bg};border:1px solid ${s.border};border-radius:10px;padding:10px 14px;cursor:${ev.tip ? "pointer" : "default"};box-shadow:${isOpen ? `0 2px 12px ${day.accentColor}20` : "none"};">
                      <div style="display:flex;justify-content:space-between;align-items:flex-start;gap:8px;">
                        <div style="flex:1;min-width:0;">
                          <div style="display:flex;align-items:center;gap:7px;margin-bottom:${ev.dur || ev.transit ? 4 : 0}px;">
                            <span style="font-size:15px;flex-shrink:0;">${escapeHtml(ev.icon)}</span>
                            <span style="font-size:13px;font-weight:700;color:${s.text};line-height:1.3;">${escapeHtml(ev.title)}</span>
                          </div>
                          ${(ev.dur || ev.transit) ? `
                            <div style="display:flex;flex-wrap:wrap;gap:8px;">
                              ${ev.dur ? `<span style="font-size:11px;color:${s.text};opacity:.75;">⏱ ${escapeHtml(ev.dur)}</span>` : ""}
                              ${ev.transit ? `<span style="font-size:11px;color:${s.text};opacity:.75;flex:1;min-width:0;">📍 ${escapeHtml(ev.transit)}</span>` : ""}
                            </div>
                          ` : ""}
                        </div>
                        ${ev.tip ? `<span style="font-size:12px;color:${s.text};opacity:.5;flex-shrink:0;margin-top:2px;">${isOpen ? "▲" : "▼"}</span>` : ""}
                      </div>
                      ${isOpen && ev.tip ? `
                        <div style="margin-top:10px;padding-top:10px;border-top:1px solid ${s.border};font-size:12px;color:#3a3a38;line-height:1.75;">
                          ${escapeHtml(ev.tip)}
                        </div>
                      ` : ""}
                    </div>
            ...
            <div style="margin-top:20px;padding:10px 14px;background:#f9f7f2;border-radius:10px;border:1px solid #e8e4dc;">
              <div style="display:flex;flex-wrap:wrap;gap:10px;align-items:center;">
                ${LEGEND.map(([k, label]) => {
                  const s = TYPE_STYLES[k];
                  return `
                    <span style="display:flex;align-items:center;gap:4px;font-size:11px;color:${s.text};">
                      <span style="width:8px;height:8px;border-radius:50%;background:${s.border};display:inline-block;"></span>
                      ${escapeHtml(label)}
                    </span>
                  `;
                }).join("")}
                <span style="margin-left:auto;font-size:10px;color:#aaa;font-style:italic;">Tap any card for insider tips</span>
              </div>
            </div>
          </div>
        </section>
  ...
  app.querySelectorAll("[data-day]").forEach((button) => {
    button.addEventListener("click", () => {
      dayIdx = Number(button.getAttribute("data-day"));
      openIdx = null;
      render();
    });
  });

  app.querySelectorAll("[data-card]").forEach((card) => {
